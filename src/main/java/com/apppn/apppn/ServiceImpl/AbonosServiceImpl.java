...
public class AbonosServiceImpl implements AbonoService {
    ...
    public void crearAbono(PagoClienteDTO pagoClientesDto) {
       ...
       for (AplicarPagoDTO aplicarPagoDTO : pagoClientesDto.getAplicarPagoDTO()) {
            Facturacion facturacion = facturacionRepository.findById(aplicarPagoDTO.getIdFacturacion()).orElseThrow(() -> new IllegalArgumentException("No existe la facturacion con id " + aplicarPagoDTO.getIdFacturacion()));
            if(Objects.isNull(facturacion)) {
                throw new IllegalArgumentException("No existe la facturacion con id " + aplicarPagoDTO.getIdFacturacion());
            }

            List<PlanPagos> planPagos = facturacion.getPlanPagos().stream().filter(pp -> pp.getFechaPago().equals(aplicarPagoDTO.getFechaPago()) && pp.getSaldo()>0).collect(Collectors.toList());
       }
    }
